...
DesinstalarPaquete() {
    ...
    if [ "$(dpkg -l "$paquete" 2>/dev/null | grep '^ii')" = "" ]; then
        ...
        read -p "¿Desea desinstalar el servicio $paquete? (s/n): " confirmacion
        if [ "$confirmacion" = "s" ] || [ "$confirmacion" = "S" ]; then
            echo "Empezando desinstalación..."
            apt-get remove -y --purge "$service" > /dev/null
            echo "Se ha terminado de desinstalar el servicio $paquete"
        else
...
ReiniciarPaquete() {
	if [ "$(dpkg -l $1 2>&1 | grep 'ii')" = "" ]; then
		...
		pkill vsftpd 2>/dev/null; sleep 1
		vsftpd /etc/vsftpd.conf &
		echo "vsftpd reiniciado."
	fi
}
...
FormatoIpValido() {
    local patron='^(((10[0-9])|(1?[1-9]?[0-9])|(2[0-4][0-9])|(25[0-5]))\.){3}((10[0-9])|(1?[1-9]?[0-9])|(2[0-4][0-9])|(25[0-5]))$'
    if ! [[ "$1" =~ $patron ]]; then
...
RestringirOctetos() {
    ...
    if [ "${_oct[0]}" -eq 0 ]; then
        echo "El primer octeto no puede ser 0, abortando..."
        exit 1
    elif [ "${_oct[0]}" -eq 127 ]; then
        echo "El primer octeto no puede ser 127 (loopback), abortando..."
        exit 1
    elif [ "${_oct[0]}" -eq 255 ]; then
        echo "El primer octeto no puede ser 255, abortando..."
        exit 1
    fi
}
...
ObtenerSegmento() {
    ...
    if [ "${_oct[0]}" -ge 1 ] && [ "${_oct[0]}" -le 126 ]; then
        echo "${_oct[0]}.0.0.0"
    elif [ "${_oct[0]}" -ge 128 ] && [ "${_oct[0]}" -le 191 ]; then
        echo "${_oct[0]}.${_oct[1]}.0.0"
    elif [ "${_oct[0]}" -ge 192 ] && [ "${_oct[0]}" -le 223 ]; then
        echo "${_oct[0]}.${_oct[1]}.${_oct[2]}.0"
    else
        echo "0.0.0.0"
    fi
}
...
IncrementarIp() {
    ...
    if [ "${_oct[3]}" -ge 256 ]; then
        _oct[3]=0; _oct[2]=$(( ${_oct[2]} + 1 ))
    fi
    if [ "${_oct[2]}" -ge 256 ]; then
        _oct[2]=0; _oct[1]=$(( ${_oct[1]} + 1 ))
    fi
    if [ "${_oct[1]}" -ge 256 ]; then
        _oct[1]=0; _oct[0]=$(( ${_oct[0]} + 1 ))
    fi
...
AplicarIpRed() {
    ...
    while read linea; do
        if [ "$_encontrado" = "true" ]; then
            _contenido="${_contenido}address $1\n"
            _reemplazado="true"
            while read linea; do
                if [[ "$linea" =~ ^[[:space:]]*(auto|iface|allow|source|\#) ]]; then
                    break
                fi
            done
        fi

        _contenido="$_contenido$linea\n"
        local _regex='[[:space:]]*iface[[:space:]]*red_sistemas[[:space:]]*'

        if [[ "$linea" =~ $_regex ]]; then
            _encontrado="true"
        else
            _encontrado="false"
        fi
    ...
    if [ "$_reemplazado" = "true" ]; then
        echo -e "$_contenido" > /etc/network/interfaces
    else
        {
            echo -e "auto red_sistemas"
            echo -e "iface red_sistemas inet static"
            echo -e "address $1"
...
ValidarArregloLleno() {
    ...
    while [ "$_i" -lt "$_total" ]; do
        if [ -z "${_arr[$_i]}" ]; then
            echo "Se encontró un valor vacío dentro del listado, abortando..."
            exit 1
        fi
        ((_i++))
    done
...
ValidarNumeroGrupo() {
    ...
    while [ "$_i" -lt "$_total" ]; do
        if [ "${_arr[$_i]}" != "1" ] && [ "${_arr[$_i]}" != "2" ]; then
            echo "El grupo debe identificarse con 1 o 2, abortando..."
            exit 1
        fi
        ((_i++))
    done
...
ValidarUsuarioNuevo() {
    ...
    while [ "$_i" -lt "$_total" ]; do
        if grep -q "${_arr[$_i]}" /etc/passwd; then
            echo "El usuario '${_arr[$_i]}' ya existe en el sistema, abortando..."
            exit 1
        fi
        ((_i++))
    done
